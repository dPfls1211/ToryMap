...
public class cam360_2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!canvasClickedCheck && iscancontroll)
        {
            if (touchcheck)
                touchscreenRotation();
            else
    ...
    void mouseclickedRotation()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            // if (transform.eulerAngles.y < minY)
            //     // original.eulerAngles = new Vector3(transform.eulerAngles.x, 40, transform.eulerAngles.z);
            //     transform.eulerAngles = transform.eulerAngles+new Vector3(transform.eulerAngles.x, minY, transform.eulerAngles.z);
            // if (transform.eulerAngles.y > maxY)
